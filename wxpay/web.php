...
*/
	...
	if($order_id=$_GET['order_id']){
		...
		if($order['pay_status']==1){
			header('location:http://'.$_SERVER['HTTP_HOST'].'/index.php?g=Weixin&m=Ucenter&a=order_detail&id='.$order_id);
		}
...
//	}
	...
	session_start();
	$uid=$order['uid'];//$_SESSION['user_id'];
	...
	$unifiedOrder->setParameter("body",$goods['goods_name']);//商品描述
	//自定义订单号，此处仅作举例
	...
	$unifiedOrder->setParameter("trade_type","MWEB");//交易类型MWEB
	//非必填参数，商户可根据实际情况选填
		...
		WeixinJSBridge.invoke(
			...
			function(res){
				...
				if(res.err_msg=="get_brand_wcpay_request:ok") {
					//支付成功，跳转至支付成功页面
					if(goods_id==126){
						var success_url="http://www.kaixin120.cn/registerUser";
					}else{
...
<body onLoad="callpay();" style="display:">
        <div id="box">
            <div id='alt'>订单信息</div>
            <div id="content">
	...
	<div align="center">
        ...
        <a class='btn' href="javascript:window.location.href='weixin://'">打开微信</a>
	</div>
</body>
<style>
    #box{
